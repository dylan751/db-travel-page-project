...
export async function getTourById(tourId) {
  const response = await fetch(`http://localhost:3000/api/tours/${tourId}`); // MySql Query lấy về 1 array
...
export async function getTourReviews(tourId) {
  const respone = await fetch(`http://localhost:3000/api/reviews/${tourId}`);
